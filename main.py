...
if __name__ == '__main__':
    ...
    parser.add_argument('--name_data',
                        default='shape2motion',
                        help='name of the dataset we use')
    parser.add_argument('--item',
                        default='eyeglasses',
                        help='name of the dataset we use')
    parser.add_argument('--num_expr',
                        default=0.01,
                        help='small set data used for testing')
    parser.add_argument(
        '--nocs_type',
        default='ancsh',
        help='whether use global or part level NOCS')  # default A/B/C
    parser.add_argument('--data_mode',
                        default='test',
                        help='how to split and choose data')

    # control model architecture
    parser.add_argument('--pred_joint',
                        action='store_true',
                        help='whether we want to predict joint offsets')
    parser.add_argument('--pred_joint_ind',
                        action='store_true',
    ...
    parser.add_argument('--early_split_nocs',
                        action='store_true',
    ...
    parser.add_argument('--test',
                        action='store_true',
                        help='Run network in test time')
    parser.add_argument('--debug',
                        action='store_true',
                        help='indicating whether in debug mode')
    parser.add_argument('--gpu',
                        default='0',
                        help='help with parallel running')
    ...
    should_restore = (ckpt is not None) and (ckpt.model_checkpoint_path
                                             is not None)
    ...
    with tf.Session(config=tf_conf, graph=net.graph) as sess:
        ...
        if is_testing:
            ...
            test_data = Dataset(root_dir=root_data,
                                ctgy_obj=args.item,
                                mode=args.data_mode,
            ...
            train_data = Dataset(root_dir=root_data,
                                 ctgy_obj=args.item,
                                 mode='train',
                                 name_dset=args.name_data,
                                 batch_size=batch_size,
            ...
            # seen instances
            val1_data = Dataset(root_dir=root_data,
                                ctgy_obj=args.item,
                                mode='test',
                                name_dset=args.name_data,
                                batch_size=batch_size,
            ...
            # unseen instances
            val2_data = Dataset(root_dir=root_data,
                                ctgy_obj=args.item,
                                mode='test',
                                name_dset=args.name_data,
                                batch_size=batch_size,
