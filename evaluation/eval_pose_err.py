...
if __name__ == '__main__':
    ...
    for key, file_name in result_files.items():
        if key == 'nonlinear':
            ...
            with open(file_name, 'rb') as f:
                ...
                basenames[key] = list(datas[key].keys())
                print('number of data for {} : {}'.format(
                    key, len(basenames[key])))
    ...
    for key, cur_data in datas.items():
        ...
        for i in range(len(basenames[key])):
            ...
            if instance in ['45841']:
                continue
            if cur_data[basename]['scale'] is None or cur_data[basename][
                    'scale'] is []:
                continue
            r_raw_err[key].append(cur_data[basename]['rpy_err'][key])
            t_raw_err[key].append(cur_data[basename]['xyz_err'][key])

    print('For {} object, {} nocs, mean rotation err per part is: '.format(
    ...
    print('For {} object, {} nocs, mean translation err per part is: '.format(
        args.domain, args.nocs))
    for item in test_items:
        num_valid = r_raw_err[item].shape[0]
        t_err = []
    ...
    print('For {} object, {} nocs, 5 degrees accuracy per part is: '.format(
        args.domain, args.nocs))
    for item in test_items:
        num_valid = r_raw_err[item].shape[0]
        r_acc = []
    ...
    print('For {} object, {} nocs, 5 degrees, 5 cms accuracy per part is: '.
        ...
        for p in range(num_parts):
            ...
            if args.item in ['drawer']:
                nocs_corner[0] = np.dot(nocs_corner[0].reshape(1, 3) - 0.5,
                                        rot_mat.T) + 0.5
                nocs_corner[1] = np.dot(nocs_corner[1].reshape(1, 3) - 0.5,
                                        rot_mat.T) + 0.5
    ...
    for i in range(len(basenames['nonlinear'])):
        ...
        try:
            ...
            for key in ['baseline', 'nonlinear']:
                # print(key)
                cur_data = datas[key]
                if basename not in cur_data or cur_data[basename][
                        'scale'] is None or cur_data[basename][
                            'scale'] is [] or np.any(
                                np.isnan(
                                    cur_data[basename]['translation'][key])):
                ...
                if key == 'nonlinear':
                    f = h5py.File(
                        my_dir + '/results/test_pred/{}/{}.h5'.format(
                            npcs_exp, basename), 'r')
                ...
                for j in range(num_parts):
                    ...
                    if nocs_pred.shape[1] == 3:
                        ...
                        centered_nocs = nocs_pred[partidx['pred'][j], 3 * j:3 *
                                                  (j + 1)] - 0.5
                    scale_pred = 2 * np.max(abs(centered_nocs), axis=0)
                    bboxs3d['pred'].append(
                        get_3d_bbox(scale_pred,
                                    shift=np.array([1 / 2, 1 / 2,
                                                    1 / 2])).transpose())
                    ...
                    shifted_part_canon = np.dot(
                        np.concatenate([
                            ...
                            np.ones((len(partidx['pred'][j]), 1))
                                       ...
                                       axis=1), np.linalg.pinv(rt_0.T))[:, :3]
                    boundary['dynam'].append(np.min(shifted_part_canon[:, 0]))
                    partidx['gt'].append(np.where(cls_per_pt_gt == j)[0])
                    scale_gt = bbox3d_all[instance][j][1][0] - bbox3d_all[
                        instance][j][0][0]
                    bboxs3d['gt'].append(
                        get_3d_bbox(scale_gt,
                                    shift=np.array([1 / 2, 1 / 2,
                                                    1 / 2])).transpose())
                    scale_err_per_part.append(
                        np.linalg.norm(scale_pred * s[j] - scale_gt * s_gt[j]))
                    volume_err_per_part.append(scale_pred[0] * scale_pred[1] *
                                               scale_pred[2] * s[j] /
                                               (scale_gt[0] * scale_gt[1] *
                                                scale_gt[2] * s_gt[j][0]) - 1)
    ...
    for key, cur_data in datas.items():
        ...
        for i in range(len(basenames['nonlinear'])):
            try:
                ...
                if cur_data[basename]['scale'] is None or cur_data[basename][
                        'scale'] is [] or np.any(
                            np.isnan(cur_data[basename]['translation']
                                     [key])) or basename not in datas['pn_gt']:
                    # print('wrong data for scale or nan: ', basename)
                    continue
                ...
                if datas['pn_gt'][basename]['rt'] is None or datas['pn_gt'][
                        basename]['scale'] is None:
                    continue
                ...
                for j in range(1, num_parts):
                    ...
                    if args.nocs == 'NAOCS' and key == 'nonlinear':
                        ...
                        t_diff_pred = boundary_all[key][basename]['dynam'][
                            j] - boundary_all[key][basename]['canon'][j]
                        t_diff_pred = np.dot(
                            r0,
    ...
    if args.item == 'drawer':
        ...
        print(
            'For {} object, {} nocs, mean relative rotation err per part is: '.
            format(args.domain, args.nocs))
        for item in test_items:
