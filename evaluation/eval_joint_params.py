...
if __name__ == '__main__':
    ...
    for i in range(len(basenames['nonlinear'])):
        ...
        try:
            ...
            translation_list = []
            for j in range(num_parts):
                nocs_pred_final['pn'][part_idx_list_pred[j], :] = nocs_pred[
                    'pn'][part_idx_list_pred[j], j * 3:j * 3 + 3]
                if nocs_pred['gn'].shape[1] == 3:
                    nocs_pred_final['gn'][
                        part_idx_list_pred[j], :] = nocs_pred['gn'][
                ...
                print('scale, translation from global NOCS to part NOCS is: ',
                      scale, translation)
            ...
            for j in range(1, num_parts):
                thres_r = 0.2
                offset = unitvec_pred * (1 -
                                         heatmap_pred.reshape(-1, 1)) * thres_r
            ...
            for j in range(1, num_parts):
                ...
                joint = {}
                joint['p'] = np.dot(s[0] * t_joint_pt.reshape(1, 3),
                                    r[0].T) + t[0]
                joint['l'] = np.dot(joints['pred'][j - 1]['l'].reshape(1, 3),
                                    r[0].T)
                print('pred joint pt: ', joint['p'], 'joint axis: ',
                      joint['l'])
            ...
            for j in range(1, num_parts):
                ...
                joint = {}
                joint['p'] = np.dot(s_g[0] * t_joint_pt.reshape(1, 3),
                                    rt_g[0][:3, :3].T) + rt_g[0][:3, 3]
                joint['l'] = np.dot(joints['gt'][j - 1]['l'].reshape(1, 3),
                                    rt_g[0][:3, :3].T)
            ...
            joints_list = []
            joints_list.append(
                [t_joints['pred'][0]['p'], t_joints['pred'][0]['l']])
            joints_list.append(
                [t_joints['gt'][0]['p'], t_joints['gt'][0]['l']])
            # plot_arrows(nocs_gt['gn'][idx], [offset[idx]], joints_list, whole_pts=input_pts, title_name='camera space: gt joint {}'.format(j))
            # start evaluation
            angle_err = []
            dist_err = []
            for j in range(1, num_parts):
                r_diff = axis_diff_degree(t_joints['gt'][j - 1]['l'],
                                          t_joints['pred'][j - 1]['l'])
                t_diff = dist_between_3d_lines(t_joints['gt'][j - 1]['p'],
                                               t_joints['gt'][j - 1]['l'],
                                               t_joints['pred'][j - 1]['p'],
                                               t_joints['pred'][j - 1]['l'])
                print('joint {} --- r_diff: {}, t_diff: {}'.format(
                    j, r_diff, t_diff))
    ...
    for k in range(num_parts - 1):
        print('joint {} with mean angle error {} degrees, mean dist {}'.format(
            k, np.mean(np.abs(r_diff_arr[:, k])),
            np.mean(np.abs(t_diff_arr[:, k]))))
        print(np.mean(np.abs(r_diff_arr[:, k])),
              np.mean(np.abs(t_diff_arr[:, k])))
