...
def render_data(data_root,
    ...
    while (simu_cnt < ARTIC_CNT):
        ...
        lastTime = time.time()
        view_num = 100
        pitch_choices = pitch_low + (pitch_high -
                                     pitch_low) * np.random.rand(view_num)
        ...
        for i in range(view_num):
            ...
            if (img_id < RENDER_NUM and _RENDER_FLAG):
                ...
                mask = img_arr[4]
                depth = 255.0 * nearPlane / (farPlane -
                                             (farPlane - nearPlane) * depth_raw
                                             )  # *farPlane/255.0
                far = farPlane
                near = nearPlane
                depth_to_save = 2.0 * far * near / (far + near - (far - near) *
                                                    (2 * depth_raw - 1.0))
                ...
                if i == 0:
                    ...
                    for joint in range(pybullet.getNumJoints(obj_parts[0])):
                        lstate = pybullet.getLinkState(
                            ...
                            linkIndex=joint,
                            computeForwardKinematics=True)
                        joint_pos[joint] = OrderedDict([(0, list(lstate[0])),
                                                        (1, list(lstate[1])),
                                                        (2, list(lstate[2])),
                                                        (3, list(lstate[3])),
                                                        (4, list(lstate[4])),
...
if __name__ == "__main__":
    ...
    parser.add_argument('--item',
                        default='eyeglasses',
                        help='name of category we use')
    parser.add_argument('--dis',
                        default=3,
                        help='default camera2object distance')
    parser.add_argument('--mode',
                        default='train',
                        help='mode decides saving folder:train/demo')
    ...
    parser.add_argument('--yaw',
                        ...
                        help='camera view angle',
                        required=True)
    parser.add_argument('--min_angles',
                        default='30,40,50',
                        help='minimum joint angles',
                        required=True)
    parser.add_argument('--max_angles',
                        default='30,40,50',
                        help='maximum joint angles',
                        required=True)
    parser.add_argument('--cnt',
                        default=30,
                        help='count of articulation change')
    parser.add_argument('--num',
                        default=10,
                        help='number of rendering per articulation')
